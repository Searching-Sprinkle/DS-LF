...
def config_parser():
    ...
    return parser
# args = parser.parse_args() ：解析命令行参数，并将其存储在 args 对象中
...
#这里导入了项目中的自定义模块，如配置加载、渲染、训练器、损失函数、各种图像质量评估指标等
...
#加载指定的配置文件，配置文件中通常包含了训练过程中的各种参数设置。
...
class BasicTrainer(Trainer):
    ...
    def compute_loss(self, data, global_step, idx_epoch):
        ...
        # 训练过程：模型的前向传播是在以下代码中执行的：
        ret = render(rays, self.net, self.net_fine, **self.conf["render"])
        # stx()
        
        projs_pred = ret["acc"]
        
        # print(f"projs_pred shape: {projs_pred.shape}")
    ...
    def eval_step(self, global_step, idx_epoch):
        ...
        if loss["psnr_3d"] > self.best_psnr_3d:
            ...
            # self.lpips = loss["lpips"]
            # self.logger.info(f"best model update, epoch:{idx_epoch}, best lpips:{self.lpips:.8g}")
